...
if($status == 1){
    ...
    $textBt = "Reabrir meta";
    $textoMeta = "<span style=\"text-decoration: line-through\">".utf8_decode($h["meta"])."</span>";
}else{
    $textBt = "Meta cumprida!!";
    $textoMeta = utf8_decode($h["meta"]);
}
...
<? if($visitante == false){ ?>
			...
			<li class="ui-state-default ui-corner-all" title="Excluir meta!!!" onclick="javascript:excluirMeta('<?= $h["id"] ?>')"><span class="ui-icon ui-icon-circle-close"></span></li>
<? }else{ ?>
